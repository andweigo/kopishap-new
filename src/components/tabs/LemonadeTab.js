...
const PRODUCTS = ALL_PRODUCTS.filter(p => typeof p.id === 'string' && p.id.startsWith('l'));
...
const LemonadeTab = ({ showAll = false, onSeeAll, searchQuery = '', suppressEmpty = false }) => {
  ...
  const productsToShow = showAll ? filtered : filtered.slice(0, 5);

  let data = [];
  if (filtered.length === 0) data = [{ id: 'empty' }];
